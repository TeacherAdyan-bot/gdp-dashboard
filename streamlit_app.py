...
# 2. Function to load the background image
...
st.markdown(f"""
    ...
    .calculator-container {{
        ...
        padding: 40px;
        border-radius: 30px;
        border: 5px solid #800000; /* Red/Maroon Border */
        box-shadow: 0px 20px 40px rgba(0,0,0,0.8);
        margin-top: 20px;
        text-align: center;
    }}
    /* Force all text inside the box to be WHITE */
    .calculator-container h1, .calculator-container h3, .calculator-container p, .calculator-container label {{
    ...
    /* Keep number inputs black text on white background for clarity */
    .stNumberInput input {{
...
# THE TITLE IS NOW MOVED HERE (INSIDE THE DIV)
st.markdown("<h1>🧪 Chemical Kinetics: Rate Law Determinator</h1>", unsafe_allow_html=True)
st.markdown("<p>Enter your experimental data to calculate reaction orders and the rate constant (k).</p>", unsafe_allow_html=True)
...
if st.button("Calculate Results"):
    ...
    try:
        ...
        st.write(f"**Order A (m):** {m} | **Order B (n):** {n}")
        st.latex(rf"Rate = {k:.4e} \ [A]^{{{m}}} [B]^{{{n}}}")
    except:
        st.error("Please check your experimental data points.")

st.markdown('</div>', unsafe_allow_html=True) # THIS ENDS THE BOX
